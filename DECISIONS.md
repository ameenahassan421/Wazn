...
rather than left as a silent contradiction, because Stage 4B is the ACTIVE
stage and its text still reads "wrap the same React codebase with Capacitor".

Two documents said not to do this and both were read first. Stage 4B names
Capacitor and keeps the PWA canonical; the v5 handoff's fidelity contract,
item 3, says recreate the screens "with the repo's own stack". Ameen overrode
both knowingly. The v5 DESIGN contract is untouched by that: the palette, the
ramp, the radii and the copy are all still binding, and they get reproduced in
NativeWind instead of Tailwind.

### The sequencing, and why the web work stops here

v5 stands at 6 of 23 build-order items. Finishing it in React and then porting
means building 17 items twice; freezing now and finishing in native builds
them once. So **no further v5 work lands in the React web app after #98.** The
enabler is §7.0: the app has not been shared, so a port window costs no user
anything. That stops being true the moment it is.

### What E1 actually did

One target, not two: Expo Router renders web through react-native-web, so
trywazn.app eventually becomes the Expo web export rather than a second
codebase. The 222 static SEO pages are generated by `build_seo_pages.mjs` and
are independent of the app shell, so F2 survives untouched.

`packages/core` is an npm workspace holding **33 modules**: the whole
calculation engine, `epley` `units` `plates` `warmup` `forecast` `readiness`
`coach-mode` `ghost-reason` `streak` `progress` `body` `commit`
`active-workout` `write-queue` and the rest, with their tests. 30 modules stay
in `src/lib`: the browser-bound layer and the data access.

**The split is enforced by the type system, not by a grep.**
`packages/core/tsconfig.json` omits `DOM` from `lib`, so `document`, `window`
and `localStorage` are compile errors inside the package. `types: ["node"]`
puts back the three ambients the core genuinely uses: `Intl` (19 modules),
`performance` (2), `fetch` (1). `npm run typecheck` runs both projects.

`supabase.ts` did NOT move. It reads `import.meta.env` at module scope and 13
modules import it, so its config injection is a design question, not a file
move, and it belongs to E1b. That is what bounds this PR to 33 modules rather
than 40.

`MUSCLE_GROUPS` and `EQUIPMENT` moved to a new `exercise-taxonomy.ts`. They are
pure vocabularies that were trapped in `exercises.ts` behind the Supabase
client; `exercises.ts` re-exports them so no caller changed.

### `check_coverage_floor.mjs` was the trap CLAUDE.md warns about

It hardcoded `src/lib`. Moving 33 modules out would have left it scanning a
smaller directory and passing while covering 33 fewer modules than the day
before. It now scans both roots and rejects a name that exists under both. It
immediately caught `exercise-taxonomy.ts`, which is the evidence it works.

### A live forecast bug, found because the move surfaced it

`weeksOfData` divided a millisecond span by a week and floored it. Lifters
train at the same wall-clock hour, so a span crossing a daylight-saving change
is an hour short of whole days: eight weeks of Monday-6pm sessions across the
March change is 55 days 23 hours, `floor(55.958 / 7)` is 7, and
`FORECAST_MIN_WEEKS` then withheld a forecast the lifter had earned. Both call
sites now snap to whole days first.

**CI could never have caught it.** The fixture builds LOCAL times, so a UTC
runner crosses no boundary; the suite was green in CI and red on any laptop in
a DST timezone. The new regression test builds the span in UTC so it fails
either way, and it was confirmed to fail against the old formula.

### Not fixed here, and it should not be lost

`e2e/offline.spec.ts:153`, "killing the tab mid-workout loses nothing", **fails
on clean `main` at d2ad091** and fails identically on this branch, so E1 did
