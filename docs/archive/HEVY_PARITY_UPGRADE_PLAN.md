...
| O15 | No routine folders/reordering UI; `routines.position` sorted on but never written. **Still true 2026-08-08, and unassigned to any phase** — note `routine_exercises.position` IS written (`routines.ts:115`); it is the routine LIST's own order that has no writer. Pairs naturally with L9 | `routines.ts:20`, 0004:42                                                                                | P (small)                                                                             |
...
- **L8. Discard exists and cannot be found. STILL OPEN — verified 2026-08-08.**
  The fix described below was never built: `Discard` appears in
  `LogScreen.tsx` and nowhere else, and `Header.tsx` has no entry for it. U1b
  shipped it, correctly
...
- **L9. Nothing knows which day is next. HALF BUILT — verified 2026-08-08.**
  B1's `session_brief()` computes the due routine by rotation and the briefing
  card says it ("Upper A is up"). The other half — the deterministic ask below
  — was not built: `routines.ts` and `RoutineList.tsx` are untouched, so the
  list sitting directly under that sentence is still in stored `position`
  order and the primary start action still is not the due day. **The card
  names a day the list does not reflect**, which is worse than neither, and it
  is now a small job rather than a new one: the SQL already exists.
  Finishing a workout returns
...
- **L3. Duration granularity. STILL OPEN — verified 2026-08-08**, the
  interval is unchanged at `30_000` (`LogScreen.tsx`), so the recommendation
  below was never applied and the decision was never recorded either way.
  Elapsed shows "42 min" on a 30s tick
