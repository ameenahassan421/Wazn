...
const F = {
  d: "'Saira Semi Condensed',sans-serif",
...
function Kick({ c, style, children }) {
  return <p style={{ ...T.kick, color: c || K.mut, margin: 0, ...style }}>{children}</p>
}
function Chip({ brass, style, children }) {
  return (
    <span
      dir="ltr"
      style={{
        ...T.meta,
        color: brass ? K.brassSoft : K.soft,
        background: brass ? K.brassBg : K.chipBg,
        borderRadius: R.chip,
        padding: '3px 8px',
        whiteSpace: 'nowrap',
...
function Btn({ kind, style, children, onClick, small }) {
  ...
  const kinds = {
    hero: { background: K.em, color: K.emInk },
    line: {
      background: 'transparent',
      color: K.text,
      boxShadow: `0 0 0 1px ${K.line2}`,
    },
    ghost: { background: 'transparent', color: K.mut },
    ink: { background: K.text, color: K.bg },
  }
  return (
    <button
      className="press"
      onClick={onClick}
      style={{ ...base, ...kinds[kind || 'line'], ...style }}
    >
      {children}
    </button>
  )
}
function Fill({ pct, brass, h }) {
  return (
    <div
      style={{
        height: h || 6,
...
function Spark({ series, w, h, project }) {
  ...
  return (
    <svg width={W} height={Hh} style={{ display: 'block', maxWidth: '100%' }}>
      <line
        x1={pad}
        y1={Hh - pad}
        x2={W - pad}
        ...
        >
          <span
            style={{
              ...
              insetInlineEnd: '20%',
              height: 2,
              background: tab === k ? K.em : 'transparent',
            }}
          ></span>
          {lab}
        </button>
...
function Header({ onAvatar, right }) {
  return (
    ...
    >
      <span style={{ fontFamily: F.d, fontWeight: 700, fontSize: 21 }} dir="ltr">
        w<span style={{ color: K.em }}>a</span>zn
      </span>
...
function RowStat({ k, v }) {
  return (
    ...
    >
      <Kick>{k}</Kick>
      <p dir="ltr" style={{ ...T.num, margin: '5px 0 0' }}>
        {v}
      </p>
    </div>
  )
...
const store = {
  get(k, d) {
    try {
      const v = localStorage.getItem('wazn5_' + k)
  ...
  set(k, v) {
    try {
      localStorage.setItem('wazn5_' + k, JSON.stringify(v))
    } catch (e) {}
  },
}
